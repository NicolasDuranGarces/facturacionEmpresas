...
public class FacturaController {
	...
	public String crearFactura(@RequestParam(value = "idProducto") int idProducto, @RequestParam(value = "cantidad") int cantidad, Model model) {
		...
		if(pedidos.size()>=0) {
			...
			if (detallePrueba!=null) {
				...
				model.addAttribute("mensaje", "Este producto ya se ha añadido al carrito y se encuentra asociado al pedido");
		...
		if (producto.getCantidadActual() >= cantidad) {
			...
			model.addAttribute("producto", producto);
			model.addAttribute("mensaje", "Actualmente solo quedan "+producto.getCantidadActual()+" unidades del producto:\n"+producto.getNombreProducto());
			return "Producto/detalleProducto";
   	...
   	public String irVerCarrito(Model model,Detallefactura detallefactura) {
		...
		try {
			...
			for (Pedido pedido : pedidos) {
				System.out.println("Pedido activo: "+pedido.getIdPedido());
			}
			pedidosActivos = true;
		} catch (Exception e) {
	    ...
	    public String confirmarPedido(Model model) {
			...
			for (int i=0; i<detalles.size(); i++) {
				...
				if (detalle.getCantidadProducto()>cantidadActual) {
					...
					ObjectError objectError = new ObjectError("ERROR_"+i, msg);
			...
			if (result.hasErrors()) {
				...
				model.addAttribute("mensaje", mensaje+"\n\n"+"Corrija la cantidad de unidades a pedir");
				return "/user/listCarrito";
			}
